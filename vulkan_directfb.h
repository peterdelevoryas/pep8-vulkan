...
typedef vk_result(VKAPI_PTR* pfn_vk_create_direct_fbsurface_ext)(
    vk_instance instance, const vk_direct_fbsurface_create_info_ext* p_create_info,
    const vk_allocation_callbacks* p_allocator, vk_surface_khr* p_surface);
typedef vk_bool32(VKAPI_PTR* pfn_vk_get_physical_device_direct_fbpresentation_support_ext)(
    vk_physical_device physical_device, uint32_t queue_family_index, idirect_fb* dfb);

#ifndef VK_NO_PROTOTYPES
VKAPI_ATTR vk_result VKAPI_CALL vk_create_direct_fbsurface_ext(
    vk_instance instance, const vk_direct_fbsurface_create_info_ext* p_create_info,
    const vk_allocation_callbacks* p_allocator, vk_surface_khr* p_surface) __asm("vkCreateDirectFBSurfaceEXT");
