...
typedef vk_result(VKAPI_PTR* pfn_vk_create_wayland_surface_khr)(vk_instance                               instance,
                                                                const vk_wayland_surface_create_info_khr* p_create_info,
                                                                const vk_allocation_callbacks*            p_allocator,
                                                                vk_surface_khr*                           p_surface);
typedef vk_bool32(VKAPI_PTR* pfn_vk_get_physical_device_wayland_presentation_support_khr)(
    vk_physical_device physical_device, uint32_t queue_family_index, struct wl_display* display);

#ifndef VK_NO_PROTOTYPES
VKAPI_ATTR vk_result VKAPI_CALL vk_create_wayland_surface_khr(
    vk_instance instance, const vk_wayland_surface_create_info_khr* p_create_info,
    const vk_allocation_callbacks* p_allocator, vk_surface_khr* p_surface) __asm("vkCreateWaylandSurfaceKHR");
